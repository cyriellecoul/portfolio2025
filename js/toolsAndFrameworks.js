const expertList = document.getElementById('expert-tools');
  ...
  function generateStars(level) {
    ...
    for(let i=0;i<5;i++) {
      starsHTML += `<i class="bi ${i<full?'bi-star-fill':'bi-star'} text-warning"></i>`;
    }
    return starsHTML;
  }
 ...
 fetch("datas/toolsAndFrameworks.json")
    ...
    .then(data => {
      data.tools.forEach((tool, idx) => {
        ...
        badge.textContent = tool.level === 'expert' ? 'Maîtrisé' :
                            tool.level === 'regular' ? 'Régulier' : 'Notion';
        ...
        setTimeout(() => {
          ...
          stars.forEach((star, i) => {
            setTimeout(() => { star.style.opacity = 1; }, i * 200);
          });
          const finalWidth = tool.level === 'expert' ? '100%' :
                             tool.level === 'regular' ? '70%' : '40%';
          let width = 0;
